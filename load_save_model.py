...
def checkpoint_save_stage(model,trainloss_to_file,testloss_to_file,train_metric_to_file,test_metric_to_file,Parameters,Model_name,Train_mode,Dataset,model2=None, checkpoint_suffix=None):
        ...
        if checkpoint_suffix is not None:
            if model2 is not None:
                torch.save(model.state_dict(),path+'/CHECKPOINT1'+checkpoint_suffix+'.t7')
                torch.save(model2.state_dict(),path+'/CHECKPOINT2'+checkpoint_suffix+'.t7')
            else:
                torch.save(model.state_dict(),path+'/CHECKPOINT'+checkpoint_suffix+'.t7')
